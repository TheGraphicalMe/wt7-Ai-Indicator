// ─── SMART AI ACCESS — CONSTANTS ─────────────────────────────────────────────

export const FEATURES = [
  {
    icon: '🌍',
    title: 'Works Across All Markets',
    desc: 'Crypto 🪙 | Gold 🟡 | Stocks 📈 | Forex 💱',
  },
  {
    icon: '📊',
    title: 'Multi-Timeframe Trend Analysis',
    desc: 'See the bigger picture & sniper entries 🎯',
  },
  {
    icon: '🔗',
    title: 'Trend Correlation',
    desc: 'Understand how markets move together 🔄',
  },
  {
    icon: '📉',
    title: 'Dynamic Trendlines for intraday',
    desc: 'Auto-adjusting for precision trading ⚡',
  },
  {
    icon: '🧱',
    title: 'Automatic Order Blocks',
    desc: 'Smart institutional zones detected 🧠',
  },
  {
    icon: '🌊',
    title: 'Monthly Liquidity Pools',
    desc: 'Track where the real money flows 💰',
  },
  {
    icon: '🔄',
    title: 'Automatic Swing Trades',
    desc: 'Catch big moves effortlessly 🚀',
  },
  {
    icon: '⚡',
    title: 'Automatic Scalping',
    desc: 'Quick entries, fast profits 💸',
  },
  {
    icon: '🌙',
    title: 'Moon Cycle Insights',
    desc: 'Unique edge using lunar cycles to identify potential turning points in the market.',
  },
  {
    icon: '🧲',
    title: 'Auto FVG',
    desc: 'Automatically detect Fair Value Gaps for precision entries 🎯',
  },
  {
    icon: '💧',
    title: 'Liquidity Finder',
    desc: 'Spot hidden liquidity pools before big market moves 🌊',
  },
  {
    icon: '⏱️',
    title: 'Volatility Timer',
    desc: 'Time your trades with market volatility cycles ⏳',
  },
  {
    icon: '💵',
    title: 'Active P/L',
    desc: 'Track your live profit and loss directly on the chart 📈',
  },
]

// ─── GOOGLE FORM CONFIG ───────────────────────────────────────────────────────
export const GOOGLE_FORM_CONFIG = {
  actionUrl: 'https://docs.google.com/forms/d/e/1FAIpQLSfk0AlkDB67V7h3ILY-Ng13HA9vXKhctPdecE58AYRk1NstPw/formResponse',
  fields: {
    name:       'entry.2116052852',
    phone:      'entry.1558582620',
    email:      'entry.1444575549',
    summary:    'entry.1060472253',
    experience: 'entry.288713975',
    priority:   'entry.60996309',
  },
}
